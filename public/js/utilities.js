...
function handleAxiosError(error, form = null) {
    console.error(error);
        ...
        if (typeof response.data == "string") {
            ...
            for (let key in response.data.errors) {
                if (
                    form
                        .find(`[name="${key}"]`)
                        .siblings(".invalid-feedback").length > 0
                ) {
                    form
                        .find(`[name="${key}"]`)
                        .addClass("is-invalid")
...
$(function () {
    ...
    $(".dataTable").on("error.dt", function () {
        toastMessage("Unauthenticated.", "error");
    });
});
...
$(document).on("click", "a[delete-btn]", function (e) {
    ...
    const dt = _self.data("datatable");
    const reload = _self.data("reload");
    const title = _self.data("title");
    const text = _self.data("text");

    window.swal
        .fire({
            title: title ?? "Are you sure?",
            text: text ?? "You want to delete this record",
            icon: "error",
...
$(document).on('campaign.open', function(e, url, params) {
    ...
    if (confirm == "yes") {
        window.swal
            .fire({
                title: "Are you sure?",
                text:
                    confirmMessage ?? "Do you really want to submit this form?",
                icon: "warning",
                showCancelButton: true,
